<?php
    ...
    if(!empty($_POST["query_type"]))
    {
        ...
        $awc_year    =  ($query_type == "by_nomination_year") ? $query_param : date('Y');
    ...
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div class="category_row row bg-secondary p-3">
            ...
            <?php 
                ...
                $winner_class = new NomineeController([
                    'param'        => $query_param,
                    'awn_category' => $awc_title,
                    'query_type'   => $query_type
                ]);
                $nominees = $winner_class->getAllNomineesByCatYear();
                if(count($nominees) > 0) {
                    foreach ($nominees as $key => $winner_data) {
                ...
                <div class="flip">
                    ...
                    <div class="flip_card_text_container" style="background-image: url(<?php echo $photo_directory; ?>)">
                        <?php if ($awn_win_status == 1): ?>
                            <span class="badge bg-success p-3 mt-1 mb-1 bouncy fs-4" title="Winner"><i class="bi bi-trophy bi-center"></i></span>
                        <?php endif ?>
                    ...
                    <div class="back">
                       ...
                       <div class="pt-2"><?php echo $awn_biography; ?></div>
                    </div>
                </div>
            </div>
            <?php
            ...
            <div class="col-12 boder_all_grey">
                <div class="show_exhausted"><span class="bg-secondary px-4 py-3 d-block w-100"><strong>No nominee(s) to display now. Try other search parameters.</strong></span></div>
            </div>
            <?php } ?>
        </div>
    </div>
